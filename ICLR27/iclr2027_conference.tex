...
zero-shot coverage on the other held-out domains. Each configuration is
...
behind on Miconic and Visitall, and it does not learn Logistics or Rovers
(3.1\% and 7.4\%). Describing symbols by their roles rather than their
...
limited, and our evaluation sets every result against a random floor to
show where it fails as clearly as where it succeeds. Schemas that are
...
policies were evaluated at the best checkpoint they reached. \gadar{}$_1$ uses the
...
All training and all evaluation ran on a single GPU per run, one
run per GPU. Training one model on seven domains takes several hours; a single-domain model takes between half an hour and two hours, so
