...
Learned policies for classical planning act without search, but each is tied to the domain it was trained on: network modules per action schema, parameters per predicate, or vocabulary-sized encodings mean a trained policy cannot even be executed on a new domain's predicates and action schemas. We present \gadar{} (Generalizing Across Domains via Action Ranking), a policy that takes the domain description as input, $\pi(s, G, D)$, and constructs grounded actions in domains never seen in training, without search. Symbols are described by the roles they play in the domain rather than by their names, in three layers that each extend the previous one: (i) a \emph{lifted domain layer} embeds $D$ in every state graph through structural signatures; (ii) a \emph{binding layer} ties each ground action's applicability to the lifted preconditions it instantiates; and (iii) a \emph{chain layer} states which schema produces what another needs and how far each schema stands from the goal. The same network therefore runs on every domain, seen or unseen. We evaluate zero-shot generalization leave-one-domain-out over eight benchmark domains. A single \gadar{} policy trained on seven domains solves 58.3\% of their test instances, one point below the 59.3\% of dedicated single-domain \gadar{} models, so one policy serves many domains at no cost in coverage; a multi-task control that keeps symbol identity in its weights, while strong on some of the domains it trains on, transfers to none of the held-out ones. Zero-shot, \gadar{} executes on every held-out domain and solves \todonum{68}\% of the large test instances of held-out Visitall, where an identically-executed untrained network solves \todonum{0}\%. Zero-shot generalization to the other seven held-out domains is limited (\todonum{0--5}\% on their test instances), and we analyze where it fails, including domains whose action schemas are structurally alike but play different roles.
...
We evaluate zero-shot generalization leave-one-domain-out over all eight
benchmark domains. One \gadar{} policy serves every domain it is trained on:
trained on seven domains, it matches the in-domain coverage of dedicated
single-domain models (58.3\% against 59.3\%). It also executes on every
held-out domain. On held-out
...
other seven held-out domains zero-shot generalization is limited, and the per-domain
results suggest where it breaks: when a held-out domain's schemas are
structurally alike but play different roles, as Logistics' truck and
airplane actions do, features that ignore symbol names may not tell them
...
\item One policy for many domains: a single \gadar{} model trained on seven
  domains matches dedicated single-domain models in-domain on average, with
  positive transfer between some domains.
...
provably lacks (their Theorem 4.3; Appendix~\ref{app:llg}).
...
\citep{gabar}. The action-centric idea also extends to partial observability
\citep{gammazero, hoo_pomdp}, varying what the policy observes while
holding the domain fixed. The most recent work remains per-domain: self-improving
...
\textbf{Large language models.} Large reasoning models degrade under symbol
renaming \citep{llm_baseline}, an invariance \gadar{} has by construction;
LLM pipelines that write or repair PDDL \citep{halo_orchestrator} leave
planning to search.
...
nodes are ground, outlined nodes lifted. The chain layer (magenta) is
shown by one \emph{enables} edge, one grounded deletes-true edge, and
goal-relevance rings.}
...
\caption{\gadar{} overview. (a) $D$, state, and goal become one graph.
(b) The ladder: \union{} (symbol one-hots), \gadarbind{} (structural
features, lifted and binding layers), \gadar{} (chain layer). (c) GNN
encoder. (d) GRU decoder: schema, then arguments, scored against node
embeddings; nothing is sized by the number of schemas or objects.}
\label{fig:system}
\end{figure*}
...
policy of \citet{gabar}; what changes is
...
\textbf{Encoder.} Features are embedded into $d{=}64$ dimensions, then
$L{=}9$ rounds update edges, nodes (attention over incoming edges), and a
global node, with skip connections to the input encodings
(Appendix~\ref{app:encoder}). The output is one embedding per node, which
the decoder ranks.
...
action as described above. Each domain in turn is held out and evaluated
on two splits.

\textbf{Protocol.} Two experiment families, run for every rung of the
ladder. (i)~\emph{Zero-shot generalization} (leave-one-domain-out, the
main experiment): for each of the eight domains, a policy is trained on the remaining seven and evaluated
on the held-out domain zero-shot, having never seen its predicates,
schemas, or objects. (ii)~\emph{In-domain, multi-domain}: the same
seven-domain policies
...
rather than serving as a matched comparison.
...
than the reference; coverage is the primary metric
(Appendix~\ref{app:metrics}). Hyperparameters are carried over from
\citet{gabar} unchanged (Appendix~\ref{app:hyper}), and renaming invariance
is verified by a unit test (Appendix~\ref{app:invariance}). In-domain
results use the checkpoint with the lowest combined training and validation
loss; zero-shot results use the training epoch that maximizes mean
zero-shot coverage on the other held-out domains.

\section{Results and Discussion}

Table~\ref{table:main_table} presents the zero-shot results for all eight
held-out domains (left) and the in-domain results (right). We
...
\caption{Coverage (\%) per domain. \emph{Zero-shot generalization}: the domain
is held out of training (leave-one-domain-out) and evaluated on its test instances,
with its training-scale instances in parentheses where measured; Rand is
uniform choice among applicable actions under the same executor.
\emph{In-domain}: test instances; ladder columns average the seven
leave-one-out policies trained on the domain, \gadar{}$_1$ is trained on
that domain alone, and GABAR$^\dagger$ is published per-domain GABAR on its
own test instances. --: not measured.}
\label{table:main_table}\label{table:indomain}
\resizebox{\linewidth}{!}{\begin{tabular}{@{}l|c|ccc||c|ccc|c@{}}
\toprule
& \multicolumn{4}{c||}{Zero-shot generalization} & \multicolumn{5}{c}{In-domain} \\
Domain & Rand & \union{} & \gadarbind{} & \textbf{\gadar{}} & GABAR$^\dagger$ & \union{} & \gadarbind{} & \gadar{} & \gadar{}$_1$ \\
\midrule
Blocks & 0.2\,{\scriptsize(1.7)} & 0.0 & 1.0 & \textbf{3.0} & 100.0 & 57.2 & 46.8 & 74.6 & 95.5 \\
Miconic & 1.1\,{\scriptsize(88.3)} & 0.0\,{\scriptsize(0.0)} & 0.0\,{\scriptsize(0.0)} & 0.0\,{\scriptsize(\textbf{36.0})} & 100.0 & 86.1 & 72.8 & 61.9 & 41.2 \\
Gripper & 0.0\,{\scriptsize(0.0)} & -- & 0.0 & 0.0 & 100.0 & 84.1 & 95.3 & 94.5 & 100.0 \\
Spanner & 43.8\,{\scriptsize(63.4)} & 0.0 & 0.0 & 0.0 & 92.0 & 48.6 & 34.0 & 47.9 & 85.4 \\
Visitall & 6.0\,{\scriptsize(99.7)} & 0.0\,{\scriptsize(0.8)} & 4.0\,{\scriptsize(16.0)} & \textbf{68.0}\,{\scriptsize(\textbf{100.0})} & 90.7 & 77.3 & 78.3 & 81.1 & 54.0 \\
Grid & 0.0\,{\scriptsize(3.0)} & 0.0\,{\scriptsize(1.0)} & 0.0\,{\scriptsize(3.1)} & \textbf{2.1}\,{\scriptsize(\textbf{12.0})} & 96.3 & 89.6 & 59.4 & 94.6 & 87.5 \\
Logistics & 0.0\,{\scriptsize(0.0)} & 0.0 & 0.0 & 0.0 & 79.0 & 18.6 & 0.3 & 2.7 & 3.1 \\
Rovers & 30.9\,{\scriptsize(75.2)} & 0.0 & 0.0 & 0.0 & 82.0 & 9.6 & 8.2 & 9.0 & 7.4 \\
\midrule
Mean & & & & & 92.5 & 58.9 & 49.4 & 58.3 & 59.3 \\
\bottomrule
\end{tabular}}
\end{table}
...
\textbf{Is the representation costly on its own? (\gadar{}$_1$ vs.\ GABAR$^\dagger$.)}
Trained on a single domain, \gadar{}$_1$ is within five
points on Blocks and Gripper and within ten on Spanner and Grid, but far
behind on Miconic and Visitall, and it reaches only 3.1\% and 7.4\% on
Logistics and Rovers. Describing symbols by their roles rather than their
names, which is what makes the policy executable on an unseen domain,
therefore costs little in four domains and a great deal in the others. On
Logistics the cost is consistent with the aliasing discussed below: its
truck and airplane schemas are structurally alike, so a representation
without names cannot separate them even when the domain is in training,
while \union{}, which keeps the names, reaches 18.6\% there.

\textbf{Does one model serve several domains? (\gadar{} vs.\ \gadar{}$_1$.)}
Every ladder column is a policy that serves seven domains at once. Averaged
over the seven such policies that train on each domain, \gadar{} solves
58.3\% of the test instances, against 59.3\% for eight single-domain
\gadar{}$_1$ models: sharing one model across seven domains costs one point
on average. The average hides transfer in both directions. Training
alongside other domains helps Visitall (81.1\% against 54.0\%), Miconic
(61.9\% against 41.2\%), and Grid (94.6\% against 87.5\%), and hurts Spanner
(47.9\% against 85.4\%) and Blocks (74.6\% against 95.5\%). Logistics and
Rovers are equally low either way, so their gap to dedicated GABAR is a
cost of the representation, not of sharing. Training cost does not favor
either form clearly: one seven-domain model trains in hours on one GPU, while dedicated models take half an hour to two hours
each and leave one artifact per domain.
...
is the stronger model on some training domains (86.1\% on in-domain Miconic
and 18.6\% on Logistics, against \gadar{}'s 61.9\% and 2.7\%) and fails
...
all. Averaged in-domain, \union{} reaches 58.9\% against \gadar{}'s 58.3\%:
giving up symbol names costs under a point on average, but it costs
Miconic and Logistics.
...
widens only the domain-independent edge-feature input. The description alone does not transfer; it must be restated in
...
the two floors. Against that standard, \gadar{} generalizes zero-shot on hard Visitall
...
\textbf{Where does zero-shot generalization fail? (The other seven domains.)} On
...
Rovers} it falls below random choice, which solves 43.8\% and 30.9\%.
Logistics illustrates one cause.
...
advances the goal in a given state. Renaming invariance equates whole
domains with identical structure; schemas that are structurally alike
\emph{within} a domain can still play different roles in it. The in-domain results point
the same way: with Logistics among its training domains, \gadar{} still
reaches only 2.7\% on it, and 3.1\% when trained on Logistics alone,
against 94.6\% on Grid. Whether
structural similarity between schemas predicts zero-shot generalization
across all eight domains we leave to a systematic analysis; Blocks, which
generalizes despite
...
domain-sized parameters. One \gadar{} policy serves seven domains at once at
the in-domain coverage of dedicated single-domain models (58.3\% against
59.3\%), and on held-out Visitall the chain layer moves zero-shot coverage of the large
test instances from \todonum{4}\% to \todonum{68}\% at plan quality
\todonum{0.97}, against \todonum{6.0}\% for random choice under the same
executor. Zero-shot generalization to the other held-out domains is
limited. Schemas that are
structurally alike but play different roles, as in Logistics, may mark a
limit that renaming-invariant features cannot cross without data from the
...
Node, edge, and global features are first embedded by
...
\subsection{Relation to GOOSE's Lifted Learning Graph}
\label{app:llg}

LLG and our lifted layer share the essential idea of the domain
...
execution harnesses. Each configuration is a single run.
...
domain, each evaluated at its lowest-combined-loss checkpoint.
\gadar{}$_1$ uses the
...
Each training and evaluation run uses a single GPU. Training one model on seven domains takes several hours; a single-domain model takes between half an hour and two hours, so
