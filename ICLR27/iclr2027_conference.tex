...
held-out domain. One \gadar{} policy serves every domain it is trained on:
...
is verified by a unit test (Appendix~\ref{app:invariance}).
...
\caption{Coverage (\%) per domain. \emph{Zero-shot}: the domain is held out
of training (leave-one-domain-out) and evaluated on its test instances,
...
\toprule
& \multicolumn{4}{c||}{Zero-shot (held-out domain)} & \multicolumn{5}{c}{In-domain} \\
