...
symbol-identity features widened to the union of the training vocabularies,
...
stands from the goal. All three share GABAR's encoder, ranking decoder, and
search-free execution unchanged, so every comparison varies the
representation, not the architecture.
...
test instances zero-shot at plan quality 0.97, against 0\% for an
...
chain layer; on training-scale instances it solves 100\%, above the 50\%
ceiling of a hand-coded goal-achievement oracle. \gadar{} is invariant by
construction to renaming of predicates, schemas, and objects, a robustness
current large reasoning models lack \citep{llm_baseline}. That invariance
is also a precise ceiling: structurally isomorphic domains with different
semantics necessarily receive identical behavior, marking where zero-shot
transfer ends and adaptation must begin.
...
autoregressively by ranking \citep{gabar}. The action-centric graph line extends to partial observability:
GammaZero converts particle beliefs into uncertainty-aware graphs for
belief-space search \citep{gammazero}, and HOO-POMDP plans object
rearrangement with hierarchical object-oriented models \citep{hoo_pomdp};
both address uncertainty within a domain, orthogonal to the cross-domain
question here. The most recent work remains per-domain: self-improving
...
GNN encoder, $L$ rounds of edge, node, and global updates, unchanged from
GABAR. (d) The GRU decoder constructs a grounded action autoregressively,
...
keeps GABAR's symbol-identity one-hots, widened to the union of the
training vocabularies: the domain stays in the weights, which is multi-task
...
Because adjacent systems differ in exactly one component, the experimental
...
step isolates the chain layer.

\subsection{Action Construction and Training}

The decoder is GABAR's, changed only where per-domain constants would
otherwise appear (Figure~\ref{fig:system}d). A GRU selects a schema by
...
supervised, as in GABAR: cross-entropy against the expert's schema and
argument choices at every state along planner-generated plans. Because
...
Leave-one-domain-out is stricter than the single fixed train/held-out
split used in prior domain-independent evaluation \citep{goose_v2}. The
\emph{hard} split is the domain's test set, with instances up to an order
of magnitude larger than training-scale problems; the \emph{easy} split is
the held-out domain's own training-scale instances, unseen because the
domain was held out, separating ``cannot handle the unseen domain'' from
``cannot scale within it.'' Training domains are evaluated on the hard
split only. Published GABAR subdivides its test problems into three size
bands and we average over them, so GABAR$^\dagger$ spans the same range;
the problem sets are not identical, so that column calibrates the gap to
dedicated training rather than benchmarking against it.
...
calibrates transfer rather than competing with it. The satisficing planner that generates our training data
\citep{fast_downward} solves 100\% of these test instances by search; as in
prior work on generalized policies it is the plan-quality reference, not a
competitor.
...
P is read within, not across, columns. Encoder and decoder
...
final setting. The structural systems train for 500 epochs and \union{} for 100, by
which point its loss has fallen an order of magnitude and it fits its
training domains completely (100\% on Miconic); its zero-shot failure is
not a budget artifact, since an unseen domain has no trained slots at any
epoch.
In-domain results use the checkpoint selected by combined training and
validation loss, one checkpoint for all eight domains; zero-shot results
evaluate every saved checkpoint on the held-out domain and report the best,
by the same rule for every rung, for the empirical reason given in the
results. Each configuration is trained once on a single NVIDIA A40 GPU,
used for training and testing alike, from a fixed seed recorded with the
run.
...
The advantage does not survive averaging: 34.8\% over the eight domains
against \gadarbind{}'s 44.7\% and \gadar{}'s 60.5\%, because \union{} is
...
competition for a shared vocabulary-sized parameterization that structural
features avoid by construction, though we have not isolated the mechanism.
A symbol-identity representation learns the domains it sees and transfers
nothing beyond their vocabulary; moving the domain description into the
input is what makes transfer possible at all.
...
We therefore snapshot at a fixed cadence, evaluate every snapshot on the
held-out domain, and report the best with its epoch, by the same rule for
every rung: the reported epoch is an experimental detail, not a free
parameter.
...
never seen in training, without search. The capability rests on the binding
layer, which connects ground applicability to lifted structure, and the
chain layer, which makes the domain's producer--consumer structure and its
distance to the goal explicit; GABAR's encoder, ranking decoder, and
search-free execution carry over unchanged. On held-out Visitall the chain
layer moves zero-shot coverage from 4\% to 68\% on the hard split at plan
quality 0.97, where the untrained control solves nothing. Past the
structural transfer ceiling, adaptation needs data from the new domain:
few-shot adaptation, lifted successor generation, richer PDDL fragments,
and probabilistic settings remain future work.
