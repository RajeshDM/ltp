...
final setting. Every system is trained to convergence on its training domains; one
eight-domain model takes seven to eight hours on a single NVIDIA A40, used
for training and testing alike. In-domain results use the checkpoint
selected by combined training and validation loss, one checkpoint for all
eight domains; zero-shot results evaluate every saved checkpoint on the
held-out domain and report the best, by the same rule for every rung. Each
configuration is trained once from a fixed seed recorded with the run.
...
\textbf{One model or eight? (GABAR$^\dagger$ vs.\ the ladder.)}
Table~\ref{table:indomain} compares objects of different kinds. The
GABAR$^\dagger$ column is eight models, one trained per domain: reading a
number from it presupposes knowing which domain the instance came from and
having trained on that domain already. Every rung of our ladder is one
model that answers for all eight. The comparison also runs the other way on
cost. GABAR trains in roughly two hours per domain, about sixteen hours and
eight artifacts for that column, each to be stored, selected among, and
retrained when the domain changes; a single eight-domain model here trains
in seven to eight hours. That is why we report \union{} and \gadarbind{} as
systems and not only as ablations: all three are multi-domain policies, a
capability none of the per-domain column has, and only \gadar{} adds
zero-shot execution on top of it. As the number of domains grows the
single-model form is the more useful artifact, independent of the
per-domain gap in any one row.
...
is the stronger model on the training domains it fits (100\% on
