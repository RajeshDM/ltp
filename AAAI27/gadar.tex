...
schemas, or objects. (ii)~\emph{In-domain}: one policy trained on all eight
domains and evaluated on each. The \emph{hard} split is the domain's test
...
appendix lists every final setting. One eight-domain model takes seven to
eight hours to train on a single NVIDIA A40. Renaming invariance is
...
\small
\centering
\caption{In-domain coverage (hard split). Ladder columns: a \emph{single}
policy trained on all eight domains, per rung. \gadar{}$_1$: the same
representation trained on that domain alone, on the same test instances,
which separates the cost of the representation from the cost of sharing one
model across eight domains. GABAR$^\dagger$: published per-domain GABAR, one
model per domain, evaluated on its own test instances.}
...
\midrule
Blocks    & 100.0 & 30.0 & 53.0 & 57.0 & \todonum{--} \\ % UNION PROVISIONAL: 57/139 of 200, refresh
Miconic   & 100.0 & 100.0 & 61.3 & 83.0 & \todonum{--} \\
Spanner   & 92.0 & 1.0 & 1.0 & 35.4 & \todonum{--} \\
Gripper   & 100.0 & 100.0 & 94.2 & 93.1 & \todonum{--} \\ % UNION PROVISIONAL: 137/137 of 173, refresh
Visitall  & 90.7 & 14.0 & 68.0 & 92.0 & \todonum{--} \\
Grid      & 96.3 & 2.1 & 70.8 & 97.9 & \todonum{--} \\
Logistics & 79.0 & 14.6 & 0.0 & 16.6 & \todonum{--} \\
Rovers    & 82.0 & 5.6 & 5.6 & 9.3 & \todonum{--} \\
\midrule
Combined  & 92.5 & 33.4 & 44.2 & 60.5 & \todonum{--} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Does one model serve eight domains? (Table~\ref{table:indomain}.)}
The GABAR$^\dagger$ column is eight models, one trained per domain and
applicable only to instances of that domain; every rung of the ladder is a
single model for all eight. A single \gadar{} policy solves
\todonum{60.5}\% of the eight test sets against 92.5\% for the eight
dedicated models, and the gap is concentrated in Blocks, Logistics, and
Rovers. \gadar{}$_1$, the same representation trained on one domain,
separates two explanations for the gap. Where \gadar{}$_1$ recovers
per-domain performance, the cost lies in sharing one model across eight
domains; where it does not, it lies in the representation: in
\todonum{k} of the eight domains \gadar{}$_1$ comes within
\todonum{5} points of GABAR$^\dagger$. Training cost does not favor either
form clearly. One eight-domain model trains in seven to eight hours on one
GPU; the eight per-domain models take about two hours each, sixteen hours
sequentially or two with eight GPUs, and leave eight artifacts to maintain.
...
is the stronger model on some training domains (100\% on in-domain
Miconic and Gripper, against \gadar{}'s 83.0\% and 93.1\%) and fails
...
all. Averaged in-domain, \union{} reaches 33.4\% against \gadar{}'s
\todonum{60.5}\%, because it is bimodal: near-perfect on Miconic and
Gripper and near-zero on Spanner and Grid. We read this as competition for
a shared vocabulary-sized parameterization, though we have not isolated the
mechanism.
...
reaches only \todonum{16.6}\% on it, against 97.9\% on Grid. Whether
...
domain-sized parameters. A single \gadar{} policy covers eight domains, and
on held-out Visitall the chain layer moves zero-shot coverage of the large
