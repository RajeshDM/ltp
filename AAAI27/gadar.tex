...
Learned policies for classical planning act without search, but each policy is tied to the domain it was trained on: network modules per action schema, parameters per predicate, or vocabulary-sized encodings mean a trained policy cannot even be executed on new predicates and action schemas. We propose \gadar{} (Generalizing Across Domains via Action Ranking), the first policy for classical planning that takes the domain description as input, $\pi(s, G, D)$, and constructs grounded actions in domains never seen in training, zero-shot and without search. Our idea is to describe symbols by the roles they play rather than by the names they carry, so that two domains whose vocabularies share nothing but whose action structure is the same become one problem, learned once and reused. The capability is built in three steps, each a contribution and each the ablation of the next: (i) a \emph{lifted domain layer} replaces symbol identity with structural signatures and embeds $D$ itself into every state graph, identifying predicates and action schemas by their declared roles (which schema uses them, as precondition or effect, at which argument slots) rather than by name; (ii) a \emph{binding layer} ties each ground action's applicability to the lifted preconditions it instantiates, expressing search-free action construction in the vocabulary of the domain description; and (iii) a \emph{chain layer} states directly that one schema produces what another needs, a relation message passing otherwise loses when it aggregates over a shared predicate, together with how far each schema stands from the goal. Across held-out domains we show that a multi-task control that keeps symbol identity in its weights matches per-domain performance on the domains it trains on yet transfers nothing to an unseen one; that the domain description alone makes the policy executable but not competent; and that the chain layer converts executability into transfer, reaching 68\% coverage on the large test instances of a held-out domain at plan quality 0.97, where an identically-executed untrained network solves none. \gadar{} is invariant to symbol renaming by construction.
...
comparison isolates one component. \union{} is the control: symbol-identity
...
Our experiments follow a leave-one-domain-out protocol over eight standard
...
\citep{gabar}. The action-centric graph line extends to partial observability:
...
(Figure~\ref{fig:system}). Encoder and decoder are inherited
unchanged from the per-domain policy of \citet{gabar}; what changes is the
graph they operate on, and we do not restate the inheritance below. We present the
...
\citep{fast_downward} is the plan-quality reference, not a competitor.
...
percentage of test problems solved within the execution bound, and
...
configuration is trained for three seeds and the results averaged.
...
& E & -- & -- & \levelA 0.8 & 0.3 & \levelA 16.0 & 0.63 & \levelD \textbf{100.0} & \pB{\textbf{0.81}} \\
& H & \levelD 90.7 & \pC{1.07} & \levelA 0.0 & -- & \levelA 4.0 & 0.42 & \levelB \textbf{68.0} & \pB{\textbf{0.97}} \\
\midrule
...
& E & -- & -- & \levelA 1.0 & 0.38 & \levelA 3.1 & \pNA{0.25} & \levelA 11.98 & \pNA{0.21} \\
...
\midrule
Blocks    & 100.0 & 41.0 & 53.0 & 57.0 \\ % UNION PROVISIONAL: 57/139 of 200, refresh
...
against \gadarbind{}'s 44.2\% and \gadar{}'s 60.5\%, because \union{} is
...
distance to the goal explicit. Neither adds a learned parameter or a new
source of supervision, so what transfers is carried by the representation
rather than by the network. On held-out Visitall the chain
