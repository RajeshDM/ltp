    ...
    for cfg_hash, metrics in data.items():
        for metric, ckpts in metrics.items():
            for entry in ckpts:
                ...
                rows.append((c.get("epoch"), str(metric), cfg_hash[:6], mtime, size,
                             c.get("validation_loss"), c.get("training_loss"),
                             os.path.basename(p)))
    for r in sorted(set(rows), key=lambda x: (str(x[3]), x[0] if x[0] is not None else -1)):
        ep, metric, h, mtime, size, vl, tl, fn = r
        ep = str(ep) if ep is not None else "?"
        vl = f"{vl:.3f}" if isinstance(vl, (int, float)) else "-"
        tl = f"{tl:.3f}" if isinstance(tl, (int, float)) else "-"
        print(f"  e{ep:<5s} {str(metric):11s} cfg{h} {mtime} {size:>8s} "
